...
module lstm_sequence_controller (
    ...
    input  [119:0] input_data, // 5 inputs x 24 bits = 120 bits (Changed from 80)
    output reg rd_input,
    input [23:0] weights,      // Weights for the LSTM - 24 bits (Changed from 16)
    output reg [6:0] weight_addr,     // Address for weight memory (0-93)
    output  [23:0] output_data, // Final output after FC layer - 24 bits (Changed from 16)
...
reg [119:0] current_input; // Register to hold current input data for processing (Changed from 80 to 120)
 // Counter for input loading (0-4 for 5 inputs)
wire [23:0] input_mux_out; // Output from input multiplexer - 24 bits (Changed from 16)
wire [23:0] weights_fin;   // 24 bits (Changed from 16)
 // ============================================================================
// Timestep Counter
// ============================================================================
...
mux_5to1 #(
    ...
    .data_in0(current_input[23:0]),    // Changed bit ranges for 24-bit
    .data_in1(current_input[47:24]),   // Changed bit ranges for 24-bit
    .data_in2(current_input[71:48]),   // Changed bit ranges for 24-bit
    .data_in3(current_input[95:72]),   // Changed bit ranges for 24-bit
    .data_in4(current_input[119:96]),  // Changed bit ranges for 24-bit
    .sel(addr_counter[2:0]), // Use lower 3 bits of addr_counter to select input
    .data_out(input_mux_out)
);
assign weights_fin = (timestep == 0) ? 24'd0 : weights;  // Changed to 24-bit zero
assign  output_data= (addr_counter < 5) ? input_mux_out : ((addr_counter > 4) && (addr_counter < 99))? weights_fin : 24'h100000; // Changed constant to 24-bit: 1.0 in S3.20
endmodule