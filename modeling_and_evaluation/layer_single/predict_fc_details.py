...
class SoCLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        # ====================================================================
        # FC LAYER DETAILED COMPUTATION: SOC = h_t @ W^T + b
        # ====================================================================
        fc_in = out[:, -1, :]  # (batch, hidden_size) — last timestep output
        
        print("\n" + "="*120)
        print("FC (LINEAR) LAYER DETAILED COMPUTATION")
        print("Formula: SOC = sum(h_t[j] * w[j]) + bias")
        print("="*120)
        print(f"fc input shape:  {fc_in.shape}")
        print(f"fc weight shape: {self.fc.weight.shape}")   # (1, 94)
        print(f"fc bias shape:   {self.fc.bias.shape if self.fc.bias is not None else None}")
        
        h_vec = fc_in[0]             # (hidden_size,)
        w_vec = self.fc.weight[0]    # (hidden_size,)
        b_val = self.fc.bias[0] if self.fc.bias is not None else torch.tensor(0.0, dtype=h_vec.dtype, device=h_vec.device)
        
        print(f"\nFC weight (all {self.hidden_size} values):")
        for i in range(0, self.hidden_size, 10):
            end_idx = min(i + 10, self.hidden_size)
            print(f"  w[{i:2d}:{end_idx-1:2d}] = {w_vec[i:end_idx].tolist()}")
        print(f"\nFC bias: {b_val.item():.10f}")
        
        print(f"\n{'j':>4} | {'h_t[j]':>14} | {'w[j]':>14} | {'h_t[j]*w[j]':>16} | {'running_sum':>16}")
        print("-"*80)
        
        running_sum = torch.tensor(0.0, dtype=h_vec.dtype, device=h_vec.device)
        for j in range(self.hidden_size):
            prod = h_vec[j] * w_vec[j]
            running_sum = running_sum + prod
            print(f"{j:4d} | {h_vec[j].item():14.8f} | {w_vec[j].item():14.8f} | {prod.item():16.8f} | {running_sum.item():16.8f}")
        
        manual_fc = running_sum + b_val
        print("-"*80)
        print(f"Dot product (sum of h_t[j]*w[j]): {running_sum.item():.10f}")
        print(f"Bias:                              {b_val.item():.10f}")
        print(f"FC output (dot + bias):            {manual_fc.item():.10f}")
        
        # Module FC output for verification
        out_module = self.fc(fc_in)  # (batch, 1)
        print(f"Module FC output (self.fc):        {out_module[0, 0].item():.10f}")
        print(f"Absolute difference:               {abs(manual_fc.item() - out_module[0, 0].item()):.12e}")
        print(f"\nPredicted SOC = {manual_fc.item():.10f}")
        print("="*120 + "\n")
